...
public class GUISelectionScreen extends Screen {
    private final LogoDrawer logoDrawer;
    private final MinecraftClient client = MinecraftClient.getInstance();
    public GUISelectionScreen(Text title, LogoDrawer logoDrawer) {
        super(title);
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (MinecraftClient.getInstance().getWindow().getScaledHeight() > 250) {
            if (client.getWindow().getScaledHeight() < 250) {
                ...
                RenderUtils.drawtext("GUI", 40, 70, 0xFFFFFFFF, true);
            }
        }
